...
layui.define([], function (exports) {
	//下行是直接将 https://github.com/xnx3/translate/blob/master/translate.js/translate.min.js 粘贴过来
    //翻译完后刷新form模块
...
*/
    ...
    translate.listener.renderTaskFinish = function(task){
        //要改为的方式，拿到具体的select来进行针对性的刷新，而不是直接强制刷新整体
        for(var hash in task.nodes){
            for(var i = 0; i<task.nodes[hash].length; i++){
                //console.log(task.nodes[hash][i].parentNode);
                var parentNode = task.nodes[hash][i].parentNode;
                //console.log(parentNode.name);
                //if('option,select,dd'.indexOf(parentNode.name) > -1){
    ...
    translate.selectLanguageTag.customUI = function(languageList){
        ...
        selectLanguage.setAttribute("lay-filter", "select-translate-language-filter");
        ...
        layui.use(['form'], function () {
            var form = layui.form;
            form.render();
            //form.render('select', 'select-translate-language-filter'); //渲染 右上角切换语种的select ui
            
            //增加 onchange 事件
            form.on('select(select-translate-language-filter)', function(data){
