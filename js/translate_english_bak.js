	...
	*/
	...
	renderTask:class{
		...
		add(node, originalText, resultText){
			...
			//console.log('task add , tasks:'+typeof(tasks)+', hash:'+hash+', this.this.taskQueue:');
		...
		execute(){
			...
			for(var hash in this.taskQueue){
				var tasks = this.taskQueue[hash];
				//console.log('task execute - tasks:'+typeof(tasks)+', hash:'+hash);
				//console.log(tasks);
				//这里很诡异，带后面抽出时间来详细跟踪
				...
				for(var node_index = 0; node_index < this.nodes[hash].length; node_index++){
					...
					for(var task_index=0; task_index<tasks.length; task_index++){
						...
						try{
							...
							console.log(e);
							console.log('error , tagName:'+tagName+', originalText:'+task.originalText+', resultText:'+task.resultText+', source text : '+this.nodes[hash][task_index].content);
						}
					}
				}
			}
	...
	execute:function(docs){
		...
		*/
		...
		for(var lang_index in fanyiLangs){ //一维数组，取语言
			...
			if(lang == 'first' || lang == 'last' || typeof(lang) != 'string'){
				continue;
			}
			...
			translate.request.post(url, data, function(data){
				...
				for(var i=0; i<translateHashArray[data.from].length; i++){
					...
					if(hash == 'first' || hash == 'last' || typeof(hash) != 'string'){
						continue;
					}
...
/*
		...
		analyse:function(language, langStrs, upLangs, upLangsTwo, charstr){
			...
			//console.log(language+','+langStrs+', upLangs:'+upLangs['language']+', upLangsTwo:'+upLangsTwo['language']+', charstr:'+charstr);
			//console.log(typeof(upLangs['language']) == 'undefined')
			var index = 0; //当前要存入的数组下标
			if(typeof(upLangs['language']) == 'undefined'){
				//第一次，那么上个还没存入值，index肯定为0
			}else{
			...
			//console.log(langStrs[language][index]+', inde:'+index+', length:'+langStrs[language].length)
...
translate.language.setLocal('english'); //设置本地语种（当前网页的语种）。如果不设置，默认就是 chinese_simplified 简体中文 
translate.setUseVersion2(); translate.selectLanguageTag.show = false; translate.to = 'chinese_simplified'; translate.execute(document.all); 