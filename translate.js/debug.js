	...
	showUIDialog: function(){
		...
		translate.debug.showUIDialog_id = msg.popups({
		    text:'<div class="ignore" style="background-color: white; color: black; width: 100%; height: 100%; overflow: auto; padding: 20px; box-sizing: border-box; border: 1px solid #dfe2e5;">'
		    	+'<button class="debug-button" onclick="window.translate.debug.mouseTracking.use()">打开简单数据调试</button><br/>'
		    	+'<button class="debug-button" onclick="msg.close(window.translate.debug.showUIDialog_id); window.translate.debug.threeD.init();">打开 3D 视觉调试</button><br/>'
...
translate.debug.threeD = {
    ...
    init: function(){
		...
		elements.forEach((el) => {
			...
			if (layerType !== 'container') {
				...
				`;
				bottom.className = 'translate-js-debug-threed-mask';
    ...
    */
	...
	showElementInfo: function(targetElement) {
		...
		//判断当前元素是否就是最底层的元素Element了，如果是，那么要显示具体的翻译信息. true 是
		let isBottomLevelElement = false;
		let targetElementChildren = targetElement.children;
		if(targetElementChildren.length === 0){
			isBottomLevelElement = true;
		}else if(targetElementChildren.length === 1){
			//判断子元素是不是3D遮罩层
			console.log(targetElementChildren[0].className)
			if(typeof(targetElementChildren[0].className) === 'string' && targetElementChildren[0].className === 'translate-js-debug-threed-mask'){
				//是3D遮罩层，那这个元素也是最底层元素
				isBottomLevelElement = true;
			}
		}
		//获取如div自身中的文本，不包含子元素中的文本。 如果div自身中有文本，那说明div本身也属于一个被翻译的node
		// 下面判断了
		...
		for(let i=0;i<translateNodes.length;i++){
			...
			for(let j=0;j<analyseGets.length;j++){
				...
				if(isBottomLevelElement || currentTranslateNodeIsThisElement){
					...
					'<br/>翻译后文本 : '+currentTranslateNode.resultText+
					'<br/>整体翻译 : '+(currentTranslateNode.whole? '是':'否'); 

					if(typeof(currentTranslateNode.lastTranslateRenderTime) === 'number' && currentTranslateNode.lastTranslateRenderTime > 0){
						translateNodesString = translateNodesString +'<br/>页面最后渲染时间 : '+translate.util.formatToHms(currentTranslateNode.lastTranslateRenderTime);	
					}
					
					/*
					if(typeof(currentTranslateNode.translateTexts) === 'object'){
						for(var originalText in currentTranslateNode.translateTexts){
							console.log(originalText)
							if (!currentTranslateNode.translateTexts.hasOwnProperty(originalText)) {
								continue;
							}
							translateNodesString = translateNodesString + '<br/>通过翻译接口:'+originalText;
						}
					}
					*/

					translateNodesString = translateNodesString +'</div>';
