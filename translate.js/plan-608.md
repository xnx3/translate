...
- `translate.waitingExecute`：保留现有执行完成判断能力；segment 返回异常时第一版不自动回退，避免引入重复 execute 风险。
...
本节只描述 `translate.js` 的前端改动。前提是 `/translate.json` 已支持前文约定的 `text: (string | string[])[]` 混合结构。

### 根源判断

当前问题的根源不在请求发送阶段，而在 DOM 扫描阶段。

现有 `translate.element.whileNodes(uuid, node)` 会递归到单个 TextNode，再由 `translate.addNodeToQueueAnalysis` 入队。`translate.whole` 当前只影响“单个 TextNode 是否整体翻译”，并没有把同一个行内上下文里的多个 TextNode 合并成一个翻译单元。

因此前端最小改动应从扫描阶段入手：在普通递归进入子节点前，先识别 whole 容器内连续的 inline TextNode，并将它们组成一个 `wholeContext`。

### 反向审查一

初始想法如果在 whole 容器收集完成后直接 `return`，会漏掉子元素上的 title、placeholder、自定义属性翻译。

优化方式：

- 收集到 TextNode 后，只给这些 TextNode 标记“本轮已由 wholeContext 处理”。
- 原有递归继续执行。
- 后续 `findNode` 或 `addNodeToQueue` 遇到已标记 TextNode 时跳过。

这样可以避免重复入队，同时不破坏原有属性翻译链路。

另一个隐患是不能直接复用 `translate.whole.isWhole(node)` 作为收集根判断。该方法会向父级查找，子节点也可能返回 true，容易导致重复收集。

优化方式：

- 新增一个内部判断，只判断“当前元素本身是否命中 `translate.whole.tag/class/id`”。
- 第一版不让 `translate.whole.isEnableAll` 进入 wholeContext 收集，避免 body 级或全页面级上下文过大。

### 另一个角度优化

从维护角度看，最容易臃肿的是新增一套平行队列或平行请求系统。

优化方式：

- 不新增全局队列。
- 不新增公开 API。
- 继续复用现有 `nodeQueue`、`translateTextArray`、`translateHashArray` 和 `renderTranslateResultItems`。
- 只在现有 nodeQueue item 上增加一个小的 `wholeContext` 字段。

边界识别第一版也保持保守：

- 只收集明确 inline 的文本流。
- `code`、`pre`、`br`、块级元素、ignore、`translate="no"` 都作为切断边界。
- 不做 CSS display 运行时计算，避免为了覆盖更多场景引入额外复杂度。

### 反向审查二

接口返回数组异常时，如果立即自动回退到 `translate.execute(textNodes)`，可能带来两个问题：

- 重复扫描同一批 TextNode。
- 如果禁用标记处理不好，可能进入重复 execute。

优化方式：

- 第一版异常时不写 DOM，只记录明确日志。
- 自动 fallback 暂不混入主链路。
- 如果后续确实需要自动 fallback，再作为独立提交做，并专门处理本轮禁用标记和循环保护。

缓存同样不应混入主链路。普通缓存只适合字符串结果；wholeContext 返回的是数组，直接复用会导致数组被字符串化或旧缓存误命中。

缓存待确认选项：

- 方案 A：第一版 wholeContext 不走缓存，改动最少。
- 方案 B：wholeContext 使用 JSON 数组缓存，缓存读写都单独兼容数组。

如果确认走方案 B，应作为独立提交实现，不能和扫描、请求、回填混在一起。
...
1. 判断当前元素本身是否命中 `translate.whole.tag/class/id`。
2. 判断当前元素是否适合 whole 上下文收集。
3. 收集当前容器内连续 inline TextNode，形成一个或多个 wholeContext group。
4. 将已收集的 TextNode 标记为本轮已处理，避免后续递归重复入队。
5. 不终止原递归，保证子元素属性翻译仍按旧逻辑执行。
6. 如果收集失败，直接回到原逻辑。
...
1. 新增内部方法 `translate.addWholeContextToQueue(uuid, segments)`。
2. 使用 `segments.map(segment => segment.originalText)` 生成 `text` 外层数组中的一个字符串数组项。
3. 语言识别按整段上下文合并文本处理。
4. 使用现有 `nodeQueue` 记录 group 与原始 TextNode 的映射。
5. hash 基于整个字符串数组项生成，并加内部前缀，避免与旧单 TextNode 缓存混用。
6. 普通节点不包含 `wholeContext`，继续走原逻辑。
...
4. 如果返回异常，不写 DOM，只记录明确日志。
5. SSE 的 batch/item 和普通 done 响应都共用这段逻辑，避免流式翻译时数组被隐式转成字符串。

### 5. 缓存阶段

wholeContext 缓存不混入主链路提交。

缓存实现需要在以下两个方案中选择：

1. 第一版 wholeContext 不走缓存。
2. wholeContext 使用 JSON 数组缓存，缓存读取、写入、异常解析单独处理。

在方案未确认前，主链路只保证不会污染 DOM，不做数组缓存。

## 最新提交拆分

### 提交一：增加 wholeContext 行内文本收集

范围：

- 只改 `translate.js`。
- 新增当前元素本身命中 `translate.whole.tag/class/id` 的内部判断。
- 新增 whole 容器内连续 inline TextNode 收集方法。
- 新增本轮已处理 TextNode 标记。
- 不接入请求、渲染、缓存。
- 不改变默认行为。
...
```text
增加 wholeContext 行内文本收集
```

提交后 push。

### 提交二：新增 wholeContext 入队能力

范围：

- 只改 `translate.js`。
- 新增内部方法 `translate.addWholeContextToQueue(uuid, segments)`。
- 复用现有 `nodeQueue` 结构。
- 只给 wholeContext item 增加必要的上下文字段。
- 普通 nodeQueue item 行为不变。

commit 信息：

```text
新增 wholeContext 分段入队能力
```

提交后 push。

### 提交三：支持 wholeContext 混合 text 请求

范围：

- 只改 `translate.js`。
- 让 `translateTextArray[lang]` 可包含字符串数组项。
- `translateHashArray[lang]` 继续与外层 `text` 下标保持一一对应。
- 请求参数仍使用现有 `text`，不新增 `segments` 参数。
- 生命周期和请求日志兼容数组项。

commit 信息：

```text
支持 wholeContext 混合 text 请求
```

提交后 push。

### 提交四：支持 wholeContext 数组回填

范围：

- 只改 `translate.js`。
- 在 `renderTranslateResultItems` 中识别 wholeContext item。
- 校验响应 `text[i]` 必须是数组且长度一致。
- 逐个回填原始 TextNode。
- SSE batch/item 和普通 done 响应共用同一逻辑。

commit 信息：

```text
支持 wholeContext 分段译文回填
```

提交后 push。

### 提交五：增加 wholeContext 异常保护

范围：

- 只改 `translate.js`。
- 接口返回类型不匹配、数量不一致时不写 DOM。
- 记录明确日志，便于定位接口或前端映射问题。
- 不在本提交中做自动 fallback，避免引入重复 execute 风险。

commit 信息：

```text
增加 wholeContext 分段异常保护
```

提交后 push。

### 提交六：实现 wholeContext 缓存策略

范围：

- 需要先确认缓存方案。
- 方案 A：第一版 wholeContext 不走缓存，只明确跳过数组缓存。
- 方案 B：wholeContext 使用 JSON 数组缓存，缓存读取、写入、异常解析都单独兼容数组。
- 该提交不得混入扫描、请求、回填主链路改动。

commit 信息根据最终方案确定，例如：

```text
支持 wholeContext 数组缓存
```

提交后 push。

### 提交七：增加 whole 行内上下文翻译验证页面
...
8. segment 返回异常后不会进入自动 execute 循环。
9. wholeContext 缓存按最终确认方案验证。
...
- 接口侧数量不一致时返回失败。
- 第一版前端不写 DOM，只记录日志。
- 自动 fallback 不混入主链路，后续如需支持再单独设计。
...
- wholeContext 数组缓存单独提交，未确认前不混入主链路。
...
- 第一版不做自动 fallback。
- 异常时只跳过该 wholeContext 的 DOM 写入并记录日志。
- 如后续要做 fallback，再单独处理本轮禁用标记和循环保护。
